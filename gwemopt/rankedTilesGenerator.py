    ...
    for telescope in params["telescopes"]:
        ...
        if not os.path.isfile(preComputedFile):
            ...
            gwemopt.rankedTilesGenerator.createTileFile(
                params, preComputedFile, radecs=config_struct["tesselation"]
            )
        ...
        tileObj = RankedTileGenerator(
            map_struct["prob"], preCompDictFiles=preCompDictFiles
        )

        ranked_tile_index, ranked_tile_probs, ipixs = tileObj.getRankedTiles(
            resolution=params["nside"]
        )
        ...
        for ii, tess in enumerate(tesselation):
            ...
            moc_struct[index] = gwemopt.moc.Fov2Moc(
...
def createTileFile(params, preComputedFile, radecs=None, tileFile=None):
    ...
    theta, phi = hp.pix2ang(
        nside, np.arange(0, npix)
    )  # Construct the theta and phi arrays
    ...
    for ii in pixelIndex:
        s = np.arccos(
            np.sin(np.pi * dec[ii] / 180.0) * np.sin(np.pi * Dec_tile / 180.0)
            + np.cos(np.pi * dec[ii] / 180.0)
            * np.cos(np.pi * Dec_tile / 180.0)
            * np.cos(np.pi * (RA_tile - ra[ii]) / 180.0)
        )
...
def getTileBounds(FOV, ra_cent, dec_cent):
    ...
    ra_down_left = ra_cent - 0.5 * (np.sqrt(FOV) / (np.cos(dec_down * (np.pi / 180.0))))
    ra_down_right = ra_cent + 0.5 * (
        np.sqrt(FOV) / (np.cos(dec_down * (np.pi / 180.0)))
    )
    ra_up_left = ra_cent - 0.5 * (np.sqrt(FOV) / (np.cos(dec_up * (np.pi / 180.0))))
    ra_up_right = ra_cent + 0.5 * (np.sqrt(FOV) / (np.cos(dec_up * (np.pi / 180.0))))
...
class RankedTileGenerator:
    def __init__(
        ...
        skymap,
        preComputed_64=None,
        preComputed_128=None,
        preComputed_256=None,
        preComputed_512=None,
        preComputed_1024=None,
        preComputed_2048=None,
        preCompDictFiles=None,
    ):
        self.skymap = skymap
        ...
        if preCompDictFiles == None:
            self.preCompDictFiles = {
                64: preComputed_64,
                128: preComputed_128,
                256: preComputed_256,
                512: preComputed_512,
                1024: preComputed_1024,
                2048: preComputed_2048,
            }
        else:
    ...
    def sourceTile(self, ra, dec, tiles):
        ...
        ID = tileData["ID"]
        s = np.arccos(
            np.sin(np.pi * dec / 180.0) * np.sin(np.pi * Dec_tile / 180.0)
            + np.cos(np.pi * dec / 180.0)
            * np.cos(np.pi * Dec_tile / 180.0)
            * np.cos(np.pi * (RA_tile - ra) / 180.0)
        )
        index = np.argmin(s)  ### minimum angular distance index
    ...
    def searchedArea(self, ra, dec, resolution=None):
        ...
        pVal = pVal[order]
        s = np.arccos(
            np.sin(np.pi * dec / 180.0) * np.sin(np.pi * dec_map / 180.0)
            + np.cos(np.pi * dec / 180.0)
            * np.cos(np.pi * dec_map / 180.0)
            * np.cos(np.pi * (ra_map - ra) / 180.0)
        )
        index = np.argmin(s)  ### minimum angular distance index
    ...
    def plotTiles(
        ...
        m.drawparallels(
            np.arange(-90.0, 120.0, 20.0),
            color="grey",
            labels=[False, True, True, False],
            labelstyle="+/-",
        )
        m.drawmeridians(np.arange(0.0, 420.0, 30.0), color="grey")
        ...
        for ii in ranked_tile_indices:
            ...
            if tileEdges:
                [
                    ...
                    ra_up_left,
                    ra_up_right,
                ] = getTileBounds(FOV, RA_tile[ii], Dec_tile[ii])
    ...
    def rankGalaxies2D(self, catalog, resolution=None):
        ...
        indices = catalogData[:, 4].astype(
            "int"
        )  ### Indices of pixels for all galaxies
        galaxy_probs = pVal[indices]  ### Probability values of the galaxies in catalog
        order = np.argsort(
            -galaxy_probs
        )  ### Sorting in descending order of probability
...
class Scheduler(RankedTileGenerator):
    ...
    def __init__(
        ...
        skymapFile,
        site="Palomar",
        tileCoord="ZTF_tiles_set1_nowrap_indexed.dat",
        utcoffset=-7.0,
    ):
        ...
        self.tiles = SkyCoord(
            ra=self.tileData["ra_center"][self.tileIndices] * u.degree,
    ...
    def tileVisibility(self, t, gps=False):
        ...
        if gps:
            ...
            time = Time(t, format="mjd")  ### else time is assumed in mjd format
    ...
    def whenThisTileSets(self, index, currentTime, duration, gps=False):
        ...
        thisTile = SkyCoord(
            ra=self.tileData["ra_center"][index] * u.degree,
            dec=self.tileData["dec_center"][index] * u.degree,
            frame="icrs",
        )  ### Tile(s)
        dt = np.arange(0, duration + 1.0, 1.0)
        times = Time(currentTime + dt, format="gps")
        altAz_tile = thisTile.transform_to(
            AltAz(obstime=times, location=self.Observatory)
        )
    ...
    def observationSchedule(
        ...
        while elapsedTime <= duration:
            [tileIndices, tileProbs, altAz_sun] = self.tileVisibility(
                eventTime, gps=True
            )
            localTime = Time(eventTime, format="gps") + self.utcoffset
                    ...
                    if tileIndices[jj] not in scheduled:
                        if tileProbs[jj] > thresholdTileProb:
                            ...
                            phaseAngle = np.arctan2(
                                ...
                                Moon.distance - Sun.distance * np.cos(sunMoonAngle),
                            )
                            illumination = 0.5 * (1.0 + np.cos(phaseAngle))

                            if verbose:
                                print("Lunar illumination = " + str(illumination))
...
def apparent_from_absolute_mag(absolute_mag, source_dist_parsec):
    ...
    """
    return absolute_mag + 5 * np.log10(source_dist_parsec / 10.0)
...
def detectability(
    ...
    if np.all(~rank_reached_mask):  # if rank cannot be reached for
        ...
        if verbose:
            print("Tile not reached in ANY allotted observation time")
    ...
    if error_data is None:
        ...
        if np.any(depthReached) is False:
            if verbose:
                print("Depth not reached in ANY allotted integration time")
