...
def get_telescope_config(telescope: str) -> dict:
    """
    Load a telescope configuration file
    ...
    -------
    dict
        a dictionary containing the telescope configuration
    """
    ...
    if "config" not in params.keys():
        ...
        for configFile in configFiles:
            ...
            if params["doSingleExposure"]:
                exposuretime = np.array(opts.exposuretimes.split(","), dtype=np.float)[
                    0
                ]

                nmag = np.log(
                    exposuretime / params["config"][telescope]["exposuretime"]
                ) / np.log(2.5)
...
def auto_rasplit(params, map_struct, nside_down):
    ...
    prob_down, distmu_down, distsigma_down, distnorm_down = ligodist.ud_grade(
        map_struct["prob"], map_struct["distmu"], map_struct["distsigma"], nside_down
    )
    ...
    if is3D:
        ...
        for ipix in range(len(map_struct["distmed"])):
            dp_dr = (
                r**2
                * distnorm_down[ipix]
                * norm(distmu_down[ipix], distsigma_down[ipix]).pdf(r)
            )
...
def optimize_max_tiles(
    ...
    for ii, max_trial in enumerate(max_trials):
        ...
        coverage_struct_hold, tile_struct_hold = gwemopt.scheduler.schedule_alternating(
            params_hold,
            config_struct_hold,
            telescope,
            map_struct_hold,
    ...
    if p_dif >= 0.1:
        ...
        while count < 20 and optimized_max > 0:
            ...
            params_hold["max_nb_tiles"] = np.array(
                [np.ceil(optimized_max)], dtype=np.float
            )
            coverage_struct, tile_struct_hold = gwemopt.scheduler.schedule_alternating(
                params_hold,
                config_struct_hold,
                telescope,
                map_struct_hold,
            ...
            if (
                p_dif > p_difs[-1] and p_dif >= 0.15
            ):  # try decreasing max-tiles if n_difs increase
                optimized_max -= 0.1 * optimized_max
                continue
            elif (p_dif > p_difs[-1]) or (
                p_difs[-1] < 0.15 and (n_equal < n_equals[-1] or repeating)
            ):
                break
